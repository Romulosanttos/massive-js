...
const Massive = function (connection, loaderConfig = {}, driverConfig = {}) {
  ...
  this.queryResult = this.driver.queryResult;
};
...
Massive.prototype.query = function (query, params = [], options = {}) {
  ...
  } else if (options.paginate && options.paginate.count) {
    promise = this.driver.query(sql, params, qrm)
      .then(result => {
        var name = options.paginate.name || "paginate";
        var prop = options.paginate.count;
        result[name] = {
